...
Jetrix has been designed to allow the addition of new commands like <tt>/who</tt> or <tt>/list</tt> easily. This document will show you the steps to create a simple command <tt>/hello</tt> that will just display the message <tt>"Hello World!"</tt>.
...
The <tt>Command</tt> interface defines the required methods that any command has to implements. We will look at these methods one by one. The first one is the <tt>getAliases()</tt> method, it returns an array of Strings containing the names used to invoke the command. For our command we will use two names, <tt>"hello"</tt> and <tt>"hi"</tt>. The first alias in the array is the default name that will be displayed in the <tt>/help</tt> list.
...
The next method is <tt>getUsage(Locale locale)</tt>, it returns a String describing the usage of the command. It is used when displaying the list of commands available on the server with <tt>/help</tt>. You can ignore the <tt>Locale</tt> parameter for now, it's used for internationalization purposes. Our command has no parameter so it's pretty straight forward :
...
The <tt>getDescription(Locale locale)</tt> method is also used for the command listing, it returns a short description of the command :
...
The <tt>getAccessLevel()</tt> defines the minimal access level required to use the command. To allow everyone to use the command it should return 0, to restrict it to operators only it would return 1.
...
To make your class available to Jetrix just copy it into the <tt>jetrix/lib</tt> directory. Starting with Jetrix 0.1.1 any <tt>.class</tt> or <tt>.jar</tt> file in this directory is automatically loaded at startup. Then you need to declare your command by editing the <tt>config.xml</tt> file, under the <tt>&lt;commands&gt;</tt> element just add this :
