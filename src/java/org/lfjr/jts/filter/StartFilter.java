...
public class StartFilter extends GenericFilter
{
    ...
    public void onPline(Message m, List out)
    {
        ...
        if (text != null && text.toLowerCase().trim().startsWith("go")) {
            ...
            while(i < 6 && doStart)
            {
                TetriNETClient player = getChannel().getPlayer(i + 1);
            ...
            if (doStart)
            {
                ...
                Message startMessage = new Message(Message.MSG_STARTGAME);
                Object[] params = { new Integer(0), getChannel().getConfig().getSettings() };
                startMessage.setParameters(params);
