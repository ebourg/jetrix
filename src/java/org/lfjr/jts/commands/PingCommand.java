...
public class PingCommand implements Command
{
    ...
    public void execute(Message m)
    {
        ...
        Message response = new Message(Message.MSG_PLAYERNUM);
        int slot = client.getChannel().getPlayerSlot(client);
        response.setParameters(new Object[] { new Integer(slot) });

        // set the client properties to be used by the PingFilter
        client.getPlayer().setProperty("command.ping", "true");
        client.getPlayer().setProperty("command.ping.time", new Long(System.currentTimeMillis()));
